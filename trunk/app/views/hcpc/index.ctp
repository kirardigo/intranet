<script type="text/javascript">
	...
	{
		code = event.element().up("td").down("input").value;
		
		window.open("/hcpc/view/" + code, "_blank");
		event.stop();
	}
	...
	document.observe("dom:loaded", function() {
		...
		$$(".viewLink").invoke("observe", "click", viewRow);
...
<?php
	echo $form->create('', array('url' => '/hcpc/index', 'id' => 'HcpcIndexForm'));
	...
	<?php
		foreach ($records as $row)
		{
			echo $html->tableCells(
				array(
					'<input type="hidden" value="' . $row['Hcpc']['code'] . '" />' .
