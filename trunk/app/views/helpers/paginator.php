<?php
	 ...
	 */
		function sortDir($model = null, $options = array()) {
			...
			if (isset($options['direction'])) {
				...
				$dir = strtolower(ifset($chunks[1], 'asc'));
			}
	 ...
	 */
		function sortableHeader($title, $key = null, $options = array(), $linkOptions = array())
		{
			$ascCssClass = 'sortasc';
			$descCssClass = 'sortdesc';
			
			// Set the key if it is the same as the title
			if (empty($key))
			{
				$key = $title;
			}
			...
			// Determine whether this is the sorted column
			...
			if ($isSorted)
			{
				$classes = ifset($options['class'], '');
				$options['class'] = ($sortDir === 'desc') ? $descCssClass : $ascCssClass;
			...
			echo $htmlHelper->tag('th', null, $options);
			echo $this->sort($title, $key, $linkOptions);
			echo '</th>';
		}
		...
		function numbers($options = array()) {
			...
			$options = array_merge(
				array(
					...
					'model' => $this->defaultModel(),
					'modulus' => '8', 'separator' => ' | ',
					'first' => null, 'last' => null, 'extendedSeparator' => ' | ... | '
				),
			(array)$options);
			...
			if ($modulus && $params['pageCount'] > $modulus) {
				...
				if ($first && $start > (int)$first) {
					if ($start == $first + 1)	{
						...
						$out .= $this->first($first, array('tag' => $tag, 'after' => $extendedSeparator));
					}
				}
				...
				if ($last && $end <= $params['pageCount'] - (int)$last) {
					if ($end + 1 == $params['pageCount'])	{
						...
						$out .= $this->last($last, array('tag' => $tag, 'before' => $extendedSeparator));
					}
				}
