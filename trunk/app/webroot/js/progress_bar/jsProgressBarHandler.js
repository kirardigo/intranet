 ...
 *****************************************************************/
	...
	// Default Options
	var defaultOptions = {
		animate: false,		// Animate the progressbar?
		showText: true,		// Show text with percentage?
		finished: false,	// Is the process finished?
		cancelled: false,	// Has this process been cancelled?
		width: 120,			// Width of the progressbar - don't forget to adjust your image too!!!
		height: 12,			// Height of the progressbar - don't forget to adjust your image too!!!
		boxImage: '/js/progress_bar/images/percentImage.png',				// Image around the progress bar
		barImage: '/js/progress_bar/images/percentImage_back1.gif', 		// Image to use in the progressbar. Can be an array of images too.
		finishedImage: '/js/progress_bar/images/percentImage_back1.png',	// Image to use when percentage hits 100%
		cancelledImage: '/js/progress_bar/images/percentImage_back_red.png'	// Image to use when cancelled
	}
		...
		JS_BRAMUS.jsProgressBar.prototype = {
			 ...
			 */
				initialize		: function(el, percentage, options) {
					...
					this.el.update(
						'<img id="' + this.id + '_percentImage" class="ProgressImage" src="' + this.options.boxImage + '" alt="0%" style="width: ' + this.options.width + 'px; height: ' + this.options.height + 'px; background-position: ' + this.initialPos + 'px 50%; background-image: url(' + this.options.barImage[this.backIndex] + '); padding: 0; margin: 0;"/>' + 
						((this.options.showText == true)?'<div id="' + this.id + '_percentText" class="ProgressText">0%</div>':''));
			 ...
			 */
				...
				processQueueEntry	: function(targetPercentage) {
					...
					if (curPercentage != newPercentage) {
						setTimeout(function() { this.processQueueEntry(targetPercentage); }.bind(this), 5);
			 ...
			 */
				_setBgPosition		: function(percentage) {
					...
					// adjust the background image and backIndex
						...
						{
							$(this.id + "_percentImage").style.backgroundImage 	= "url(" + this.options.finishedImage + ")";
						}
						else if (this.options.cancelled)
						{
							$(this.id + "_percentImage").style.backgroundImage 	= "url(" + this.options.cancelledImage + ")";
						}
						else if ((newBackIndex != this.backIndex) && (this.options.barImage[newBackIndex] != undefined))
						{
